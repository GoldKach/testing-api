...
export async function submitOnboarding(req: Request, res: Response) {
  try {
    ...
    if (!payload.nationalIdUrl || !payload.bankStatementUrl) {
      return res.status(400).json({ 
        error: "Required documents missing. Please upload National ID/Passport and Bank Statement." 
      });
    }
    ...
    // Dates
    const dateOfBirth = parseDate(payload.dateOfBirth) ?? new Date("1900-01-01T00:00:00Z");
    const incorporationDate = parseDate(payload.incorporationDate);
    const establishmentDate = parseDate(payload.establishmentDate);
    ...
    const data: Prisma.EntityOnboardingUncheckedCreateInput = {
      ...
      ownershipPercentage: payload.ownershipPercentage ?? null,
